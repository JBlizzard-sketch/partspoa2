...
def main():
    st.title("🚗 Auto Parts Browser")
    st.markdown("Browse and search auto parts from Kenyan suppliers")
    
    df = load_data()
    
    if df is None:
        st.error("❌ No data found. Please run 'python scrape_all.py' first to collect data.")
        st.info("Run the scraper to fetch data from all 5 websites.")
        return
    
    st.success(f"✓ Loaded {len(df)} products from {df['supplier'].nunique()} suppliers")
